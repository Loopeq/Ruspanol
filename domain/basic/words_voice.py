...
from domain.keyboards.keyboards import inline_words_kb, inline_return_to_section_kb, \
    inline_delete_voice
...
async def cmd_return_to_sections(callback: CallbackQuery):
    await callback.message.edit_text(text=Strings.available_sections, reply_markup=inline_section_kb(get_sections(), page=0))
    await callback.answer()